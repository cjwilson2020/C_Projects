...
void init_cache() {          
	...
	for (int j = 0; j < E; j++){
	    (*(cache + i) + j)->valid = 0; //The j'th line of the i'th set of cache_t
	    (*(cache + i) + j)->tag = 0;
 ...
 */                    
void free_cache() {             
    for(int i = 0; i < S; i++){ //Iterate through sets in cache
	for(int j = 0; j < E; j++){ //Iterate thorugh lines in each set
	    //Free each line first
	    free((*(cache+i)+j));	
	}
    }
    free(cache); //free each set next
}  
...
//	printf("ACCESS_DATA(%llx); ---------\n", addr);

	//m = 64bits = 8bytes
	////therefore there are M = 2^m = 2^64 unique addresses
...
//	printf("ITERATION START----\n");
	...
	unsigned long long int set_index = addr >> b; //bit shifts the # of offsets to get the set index

	unsigned long long int line_tag = addr >> (s + b); //bit shifts the # of offsets to get the set index

	//Look through lines in set
	for( int j = 0; j < E; j++){	
	    if((cache[set_index][j]).valid != '1'){
		miss_cnt++;	
	    } else{
